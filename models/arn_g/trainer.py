...
from models.arn_g.model import Generator, Discriminator
from models.arn_g.loss import DiscriminatorLoss, GeneratorLoss
from utils.util import save_arn_models
from utils.plot import plot_ARN_loss

class ARN_G(nn.Module):
    def __init__(self, params):
        super(ARN_G, self).__init__()

        self.params = params
        self.device = self.params['device']

        self.D = Discriminator(nc = self.params['nc']).to(self.device)
        self.G = Generator(device = self.device, nf_in = self.params['nc']).to(self.device)
        ...
        self.g_loss = GeneratorLoss(self.device, self.params['discreteCol'], self.params['selected_columns'],
                                    self.params['index'])
    ...
    def D_step(self, true_data, step):
        ...
        logits, _, _, sampled_data = self.G(true_data, self.params['discreteCol'],self.params['selected_columns'],
                                            self.params['index'], self.temperature)
    ...
    def G_step(self,true_data, step):
        ...
        logits, z_mean, z_logvar, sampled_data = self.G(true_data, self.params['discreteCol'],
                                                        self.params['selected_columns'], self.params['index'],
                                                        self.temperature)
        fake_pred = self.D(sampled_data)

        gen_loss_batch, bce_loss, rec_loss, kl = self.g_loss(true_data, fake_pred, sampled_data, z_mean, z_logvar,
    ...
    def train(self, data_loader, val_loader, path_G, path_D, batch_size = 32, num_epochs = 10, step = 10, lowerbnd=5e-15,
              num_q_steps = 1, num_g_steps = 1, show_loss = True):
