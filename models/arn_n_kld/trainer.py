...
from models.arn_n_kld.model import Discriminator, Generator
from models.arn_n_kld.loss import DiscriminatorLoss, GeneratorLoss
from utils.util import save_arn_models
from utils.plot import plot_ARN_KLD_loss

class ARN_N_KLD(nn.Module):
    def __init__(self, params):
        super(ARN_N_KLD, self).__init__()
        ...
        self.d_loss = DiscriminatorLoss(device = self.device)
        self.g_loss = GeneratorLoss(device = self.device)
    ...
    def train(self, data_loader, val_loader, path_G, path_D, batch_size = 32,
              num_epochs = 10, step = 10, lowerbnd=5e-15, num_q_steps = 1,
              num_g_steps = 1, show_loss = True):
        ...
        results = {'d_losses': d_losses[:epoch], 'g_losses': g_losses[:epoch],
                   'rec_losses': rec_losses[:epoch], 'bce_losses': bce_losses[:epoch],
