...
from models.arn_g_kld.model import Discriminator, Generator
from models.arn_g_kld.loss import DiscriminatorLoss, GeneratorLoss

from utils.plot import plot_ARN_KLD_loss
from utils.util import save_arn_models

class ARN_G_KLD(nn.Module):
    def __init__(self, params):
        super(ARN_G_KLD, self).__init__()
        ...
        self.g_loss = GeneratorLoss(self.device, self.discreteCol, self.selected_columns, self.index)
    ...
    def D_step(self,true_data, step):
        ...
        logits, sampled_data = self.G(true_data, self.discreteCol, self.selected_columns,
    ...
    def G_step(self,true_data, step):
        ...
        gen_loss_batch, bce_loss, rec_loss = self.g_loss(true_data, fake_pred, sampled_data, self.temperature)
    ...
    def train(self, data_loader, test_loader, path_G, path_D, batch_size = 32, num_epochs = 10, step = 10,
              lowerbnd=5e-15, num_q_steps = 1, num_g_steps = 1, show_loss = True):
        ...
        if show_loss:
            plot_ARN_KLD_loss(d_losses[:epoch], g_losses[:epoch], d_losses_val[:epoch], bce_losses[:epoch], rec_losses[:epoch], real_scores[:epoch], fake_scores[:epoch])

        results = {'d_losses': d_losses[:epoch], 'g_losses': g_losses[:epoch], 'rec_losses':rec_losses[:epoch],
                   'bce_losses': bce_losses[:epoch], 'real_scores': real_scores[:epoch],
